...
class HomeList extends Component {
  static defaultProps = {
    ...
    // newsUrl:"http://c.3g.163.com/nc/article/list/T1467284926140/0-20.html",
  ...
  showDetail(item) {
    ...
    this.props.navigation.navigate('Detail', { id: item.postid, title: item.title });
  }
  //下拉刷新数据
  _onRefresh() {
...
function renderIcon(tab, component) {
  ...
  return <Image source={{ uri: 'https://public.shiguanghai.top/blog_img/' + iconSrc + '.png' }} style={{ width: 30, height: 30 }} />
  // return <Image source={{uri:'asset:/TabBar/'+iconSrc+'.png'}} style={{width:30,height:30}} />
  // let url = `./src/assets/TabBar/${iconSrc}.png`
  // return <Image source={require('./src/assets/TabBar/${iconSrc}.png')} style={{width:30,height:30}} />
}
