...
public class CombatSystem extends AbstractCombat {
    ...
    public int roleAttackEnemy(int AtkRole) {//角色攻击敌人
        ...
        if (enemy.getHp()>0){//敌人存活
            ...
            JOptionPane.showMessageDialog(null,"你杀死了对方并获得了大量经验！","提示消息",JOptionPane.WARNING_MESSAGE);
    ...
    public int EnemyAttackRole(int AtkEnemy) {//敌人攻击角色
        ...
        if (hurt>0){
            ...
            if(role.getCurrenthp()>0){
                JOptionPane.showMessageDialog(null,"您现存血量："+role.getCurrenthp()+"/"+role.getMaxhp(),"提示消息",JOptionPane.WARNING_MESSAGE);
                System.out.println("您现存血量："+role.getCurrenthp()+"/"+role.getMaxhp());
            }
        }
        if (role.getCurrenthp()>0){
            return 1;
